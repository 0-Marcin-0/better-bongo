...
namespace custom {
Json::Value left_key_value, right_key_value, wave_key_value;
int offset_x, offset_y;
int paw_r, paw_g, paw_b, paw_a;
int paw_edge_r, paw_edge_g, paw_edge_b, paw_edge_a;
double scale;
bool is_mouse, is_left_handed;
sf::Sprite bg, up, left, right, device, wave;
...
bool init() {
    // getting configs
    Json::Value config = data::cfg["binds"];

    is_mouse = config["mouse"].asBool();

    paw_r = config["paw"][0].asInt();
    paw_g = config["paw"][1].asInt();
    paw_b = config["paw"][2].asInt();
    paw_a = config["paw"].size() == 3 ? 255 : config["paw"][3].asInt();

    paw_edge_r = config["pawEdge"][0].asInt();
    paw_edge_g = config["pawEdge"][1].asInt();
    paw_edge_b = config["pawEdge"][2].asInt();
    paw_edge_a = config["pawEdge"].size() == 3 ? 255 : config["pawEdge"][3].asInt();

    bool chk[256];
    std::fill(chk, chk + 256, false);
    left_key_value = config["left"];
    for (Json::Value &v : left_key_value) {
        chk[v.asInt()] = true;
    }
    right_key_value = config["right"];
    for (Json::Value &v : right_key_value) {
        if (chk[v.asInt()]) {
            data::error_msg("Overlapping keybinds", "Error reading configs");
            return false;
        }
    }
    wave_key_value = config["wave"];
    ...
    // importing sprites
    up.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/up.png"));
    left.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/left.png"));
    right.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/right.png"));
    wave.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/wave.png"));
    if (is_mouse) {
        bg.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/mousebg.png"));
        device.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/mouse.png"), true);
    } else {
        bg.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/tabletbg.png"));
        device.setTexture(data::load_texture("/home/np/Projects/better-bongo/img/tablet.png"), true);
    }
